...
trainFirstLayer <- function(feature_data, labels, k = 5) {
    ...
    for (feature_name in names(feature_data)) {
        ...
        for (i in 1:nrow(param_grids$glm)) {
            ...
            results[[paste0("glm_", i)]] <- list(
                model = cv_fit, auc = auc,
...
getBasePredictions <- function(base_models, feature_data) {
...
trainSecondLayer <- function(base_predictions, labels, n_models = 10) {
    ...
    for (algorithm in c("glm", "xgboost", "rf")) {
        ...
        for (i in 1:nrow(params)) {
            ...
            if (algorithm == "glm") {
                ...
                model <- randomForest(
                    x = base_predictions, y = as.factor(labels),
                    mtry = params$mtry[i],
...
predictStackModels <- function(stack_models, new_data) {
    ...
    # Iterate through each base model and make predictions
    for (model_name in names(stack_models)) {
        model_info <- stack_models[[model_name]]
        model <- model_info$model
        if (inherits(model, "cv.glmnet")) {
            pred <- predict(model,
                newx = as.matrix(feature_matrix),
    ...
    final_prediction <- model_predictions / length(stack_models)
